...
import { getCookie, setCookie } from "@utils/browser.ts";
...
export const CookieConsent = component$(() => {
    ...
    const state = useStore({isVisible: false, isPreferenceActive: false});

    // Equivalent to useEffect in React / onMount in Astro
    useVisibleTask$(() => {
        console.log("heee: ", getCookie('cookie-consent'));
        state.isVisible = getCookie('cookie-consent') === 'accepted';
    ...
    const handleAccept$ = $(() => {
        ...
        state.isVisible = true;
        // Initialize analytics could go here
    });
    ...
    const handleReject$ = $(() => {
        ...
        // Ensure analytics aren't running
    });

    return (
        <div id="cookie-consent" class={`cookie-banner w-4/5 mb-6 m-auto ${state.isPreferenceActive ? 'rounded-xl' : 'rounded-full'} ${!state.isVisible ? 'hidden' : ''}`}>
            ...
            <div class={`cookie-content transition-all ease-in duration-300 p-4 justify-around items-center gap-4 ${state.isPreferenceActive ? '' : ''}`}>
                ...
                <div class={`transition-all duration-200 ${state.isPreferenceActive ? 'opacity-100 max-h-96' : 'opacity-0 max-h-0 overflow-hidden'}`}>
                    {/*<div class="flex p-10 transition-all ease-in duration-200">*/}
