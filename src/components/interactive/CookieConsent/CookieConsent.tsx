...
import styles from './cookie-consent.css?inline';
import {
...
import { CookieAccordion } from "@components/interactive/CookieConsent/CookieAccordion.tsx";

const BUTTON_CLASSES = "rounded-md bg-slate-800 py-2 px-4 border border-transparent text-center text-sm text-white transition-all shadow-md hover:shadow-lg focus:bg-slate-700 focus:shadow-none active:bg-slate-700 hover:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none";

export const CookieConsent = component$(() => {
    useStylesScoped$(styles);
    const state = useStore({isVisible: false, isPreferenceActive: false, currentPath: "" });

    // Equivalent to useEffect in React
    useVisibleTask$(() => {
        const cookie = getCookie('cookie-consent');
        state.isVisible = !cookie;
        if (cookie === 'accepted') enableGa();
        ...
        <div id="cookie-consent" class={[
            ...
            <div class={`cookie-content transition-all ease-in duration-300 p-4 justify-around items-center gap-4 ${state.isPreferenceActive ? '' : ''}`}>
                ...
                <div class={[
                    ...
                    <CookieAccordion accordionData={
                        [
                    {
                        title: "What is Qwik?",
                        content: "Qwik is a new kind of web framework that can deliver instant loading web applications at any size or complexity. Qwik achieves this through two main strategies: Delay execution and lazy loading of JavaScript, and Resumable instead of replayable applications."
                    },
                    {
                        title: "How does Qwik differ from other frameworks?",
                        content: "Unlike other frameworks that require JavaScript to be downloaded and executed before the page becomes interactive, Qwik delivers HTML that is immediately interactive. JavaScript is only downloaded when user interactions require it."
                    },
                    {
                        title: "What are the benefits of using Qwik?",
                        content: "Key benefits include instant page loads regardless of application complexity, excellent SEO and Core Web Vitals scores, automatic code splitting, and progressive JavaScript loading based on user interactions."
                    },
                    {
                        title: "Is Qwik production ready?",
                        content: "Yes! Qwik has reached stable v1.0 and is being used in production by many companies. It has a growing ecosystem of integrations and community support."
                    }
                        ]
