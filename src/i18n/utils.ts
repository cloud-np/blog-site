...
import type { CookieCategory } from '../libs/cookies';
...
type DeepKeys<T> = T extends object
	? {
		...
		? K | (T[K] extends { el: string; en: string } & Record<string, any>
			? {
...
export function getTranslatedCookieCategories(lang: keyof typeof languages): (CookieCategory & { displayName: string })[] {
	...
	return [
		{
			...
			isActive: true,
			cookies:
				Object.keys(ui.cookie.necessary.cookies).map(key => ({
					name: key,
		...
		},
		{
			...
			isActive: false,
			cookies:
				Object.keys(ui.cookie.analytics.cookies).map(key => ({
					name: key,
