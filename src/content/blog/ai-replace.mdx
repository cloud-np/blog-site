...
Its no secret that utilizing AI tools in some extent provides a productivity boost and speed unmatched to what
was ever capable up until now. Although even that setence is being heavily challenged by people arguing that the productivity boost and extra velocity are translated into poor quality and an amalgamation of all sorts of "bad taste" code, security vulnerabilities, and poor architectural decisions.

Nevertheless many of us have this feeling that we need to integrate it more somehow in our workflows either because of FOMO (fear of missing out) or because we actually manage be more productive with to a certain degree
because we managed to delegate certain work to it successfully.

<H2 title="What should we delegate to AI?" />
But what should we really delegate to AI? Does it really matter how and in which workflows we
really integrate these tools?
...
I mean technically given enough iterations maybe either by pure chance
or struggle AI could manage to satisfy our needs when it comes to a task relatively ok'ish they are after all probabilistic.
<Note title="Note">
...
Sure one could argue our job becomes more **reactive** vs **generative** and the creativeness slowly fades since we now reacting on issues produces from the LLM output rather than thinking but who cares our job just become easier right?
<div class="flex justify-center items-center">
	<Image src={easier} alt="ai-meme" class="w-100" />
</div>

<H2 title="Using AI for productivity" />

<H3 title="Example use" />
Let's consider a scenario where we travel back in time and our website used to look like this:

<H4 title="Our good state" />
